...
Audited on 2026-08-13. The current 15 FPS RTSP rows prove that a platform can
...
- Hailo's current async probe uses one per-stream atomic start timestamp, so a
  later buffer can overwrite the previous buffer's start. Probe latency is
  marked measurement-risk until starts are paired by PTS/offset or metadata.
