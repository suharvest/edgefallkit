...
FallOutput FallDetector::update(const FallObservation& o) {
    ...
    if (!o.valid) {
        ...
        // immediately after impact. Missing/invalid observations may retain
        // or expire state, but must never originate a new event.
        ...
        if (!initialized_) {
            ...
            // no preceding motion/history, posture alone is not a fall and
