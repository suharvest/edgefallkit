...
static_assert(temporal_yolov8s_int8_weights::kWindow == temporal_weights::kWindow);
static_assert(temporal_yolov8s_int8_weights::kFrameDim == temporal_weights::kFrameDim);
static_assert(temporal_yolov8s_int8_weights::kFeatureDim == temporal_weights::kFeatureDim);
static_assert(temporal_yolov8m_int8_weights::kWindow == temporal_weights::kWindow);
static_assert(temporal_yolov8m_int8_weights::kFrameDim == temporal_weights::kFrameDim);
static_assert(temporal_yolov8m_int8_weights::kFeatureDim == temporal_weights::kFeatureDim);
...
constexpr int kJetsonInt8sMaxHiddenDim = kJetsonMaxHiddenDim > temporal_yolov8s_int8_weights::kHiddenDim
    ? kJetsonMaxHiddenDim : temporal_yolov8s_int8_weights::kHiddenDim;
constexpr int kJetsonInt8MaxHiddenDim = kJetsonInt8sMaxHiddenDim > temporal_yolov8m_int8_weights::kHiddenDim
    ? kJetsonInt8sMaxHiddenDim : temporal_yolov8m_int8_weights::kHiddenDim;
...
WeightView weightsFor(TemporalProfile profile) {
    if (profile == TemporalProfile::YoloV8sInt8Pose) {
        return {temporal_yolov8s_int8_weights::kHiddenDim,
                temporal_yolov8s_int8_weights::kThreshold,
                temporal_yolov8s_int8_weights::kConsecutive,
                temporal_yolov8s_int8_weights::kFrameMask,
                temporal_yolov8s_int8_weights::kMean,
                temporal_yolov8s_int8_weights::kScale,
                temporal_yolov8s_int8_weights::kW1,
                temporal_yolov8s_int8_weights::kB1,
                temporal_yolov8s_int8_weights::kW2,
                temporal_yolov8s_int8_weights::kB2};
    }
    if (profile == TemporalProfile::YoloV8mInt8Pose) {
        return {temporal_yolov8m_int8_weights::kHiddenDim,
                temporal_yolov8m_int8_weights::kThreshold,
                temporal_yolov8m_int8_weights::kConsecutive,
                temporal_yolov8m_int8_weights::kFrameMask,
                temporal_yolov8m_int8_weights::kMean,
                temporal_yolov8m_int8_weights::kScale,
                temporal_yolov8m_int8_weights::kW1,
                temporal_yolov8m_int8_weights::kB1,
                temporal_yolov8m_int8_weights::kW2,
                temporal_yolov8m_int8_weights::kB2};
    }
    if (profile == TemporalProfile::Hailo8YoloV8sPose) {
        return {temporal_hailo_weights::kHiddenDim,
                temporal_hailo_weights::kThreshold,
                temporal_hailo_weights::kConsecutive,
                temporal_hailo_weights::kFrameMask,
                temporal_hailo_weights::kMean,
                temporal_hailo_weights::kScale,
                temporal_hailo_weights::kW1,
                temporal_hailo_weights::kB1,
                temporal_hailo_weights::kW2,
                temporal_hailo_weights::kB2};
    }
    if (profile == TemporalProfile::Yolo11mPose) {
        return {temporal_yolo11m_weights::kHiddenDim,
                temporal_yolo11m_weights::kThreshold,
                temporal_yolo11m_weights::kConsecutive,
                temporal_yolo11m_weights::kFrameMask,
                temporal_yolo11m_weights::kMean,
                temporal_yolo11m_weights::kScale,
                temporal_yolo11m_weights::kW1,
                temporal_yolo11m_weights::kB1,
                temporal_yolo11m_weights::kW2,
                temporal_yolo11m_weights::kB2};
    }
    return {temporal_weights::kHiddenDim, temporal_weights::kThreshold,
            temporal_weights::kConsecutive, temporal_weights::kFrameMask,
            temporal_weights::kMean, temporal_weights::kScale,
            temporal_weights::kW1, temporal_weights::kB1,
            temporal_weights::kW2, temporal_weights::kB2};
}
...
TemporalPrediction TemporalClassifier::update(const TemporalFrame& frame, double timestamp_sec) {
    ...
    const WeightView weights = weightsFor(profile_);
    TemporalFrame masked = frame;
    for (std::size_t i = 0; i < masked.size(); ++i) {
        masked[i] *= weights.frame_mask[i];
    }
    ...
    if (prediction.evaluated) {
        ...
        positive_run_ = last_probability_ >= weights.threshold
            ? positive_run_ + 1 : 0;
        last_positive_ = positive_run_ >= weights.consecutive;
    }
...
float TemporalClassifier::evaluate(double timestamp_sec) const {
    ...
    for (int d = 0; d < temporal_weights::kFeatureDim; ++d) {
        normalized[d] = (feature[d] - weights.mean[d]) /
                        std::max(weights.scale[d], 1e-12f);
    }
    std::array<float, kMaxHiddenDim> hidden{};
    for (int h = 0; h < weights.hidden_dim; ++h) {
        float value = weights.b1[h];
        for (int d = 0; d < temporal_weights::kFeatureDim; ++d) {
            value += normalized[d] * weights.w1[d * weights.hidden_dim + h];
        }
        hidden[h] = std::max(0.0f, value);
    }
    float logit = weights.b2;
    for (int h = 0; h < weights.hidden_dim; ++h) {
        logit += hidden[h] * weights.w2[h];
    }
