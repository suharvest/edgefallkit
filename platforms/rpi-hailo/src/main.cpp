...
struct App;
struct Stream {std::string id,url;GstElement*pipeline=nullptr;rpi_hailo::Tracker tracker;App*app=nullptr;uint64_t frames=0,global_event_id=0;double started=now(),latency_sum=0;std::atomic<double> input_at{0};};
struct App {
#ifdef HAVE_MOSQUITTO
...
#else
  (void)topic; if(s.frames<=2) std::cout<<json<<"\n";
#endif
...
gboolean finish(gpointer data){auto*a=static_cast<App*>(data);for(auto&sp:a->streams){double sec=now()-sp->started;std::cout<<"BENCHMARK stream="<<sp->id<<" frames="<<sp->frames<<" seconds="<<sec<<" fps="<<(sp->frames/sec)<<" mean_pipeline_ms="<<(sp->frames?sp->latency_sum/sp->frames:0)<<"\n";}g_main_loop_quit(a->loop);return G_SOURCE_REMOVE;}
}
...
App app;app.topic=env("MQTT_TOPIC","recamera/fall-detection/results/{stream_id}");app.score=std::stof(env("SCORE_THRESHOLD","0.35"));app.kpt=std::stof(env("KEYPOINT_THRESHOLD","0.25"));
  ...
  app.loop=g_main_loop_new(nullptr,FALSE);for(auto&sp:app.streams){std::ostringstream q;if(sp->url.rfind("test://",0)==0)q<<"videotestsrc is-live=true pattern=ball ! video/x-raw,framerate=30/1 ";else if(sp->url.rfind("file://",0)==0)q<<"uridecodebin uri=\""<<sp->url<<"\" ! videorate ! video/x-raw,framerate=15/1 ";else q<<"rtspsrc location=\""<<sp->url<<"\" latency="<<latency<<" protocols=tcp ! rtph264depay ! h264parse ! decodebin ";
    q<<"! videoconvert ! videoscale ! video/x-raw,format=RGB,width=640,height=640 ! queue max-size-buffers=2 leaky=downstream ! identity name=pre ! hailonet name=net hef-path=\""<<hef<<"\" scheduling-algorithm=1 vdevice-group-id=fall-shared ! fakesink sync=false";
  ...
  int seconds=std::stoi(env("BENCHMARK_SECONDS","0"));if(seconds>0)g_timeout_add_seconds(seconds,finish,&app);g_main_loop_run(app.loop);
